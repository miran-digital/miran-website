  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/v1/auth/register") {
      ...
      return sendJson(
        res,
        201,
        core.register({ email: body.email, password: body.password }),
      );
    }
    ...
    const addressDefaultMatch = url.pathname.match(
    ...
    if (req.method === "PATCH" && addressDefaultMatch) {
      return sendJson(
        res,
        200,
        addresses.setDefault(user.id, addressDefaultMatch[1]),
      );
    }
    ...
    if (
      req.method === "POST" &&
      /^\/v1\/admin\/sellers\/[^/]+\/approve$/.test(url.pathname)
    ) {
      ...
      error.code === "UNAUTHORIZED"
        ...
        : error.code === "FORBIDDEN"
          ...
          : error.code === "NOT_FOUND"
            ? 404
            : error.code === "OUT_OF_STOCK" || error.code === "CONFLICT"
              ? 409
