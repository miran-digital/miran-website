...
function makeAdmin(db, core, email) {
  const user = core.register({ email, password: "very-secure-pass-123" });
  db.prepare("UPDATE users SET role='ADMIN' WHERE id=?").run(user.id);
  return { ...user, role: "ADMIN" };
}

test("passwords are hashed, sessions store token hashes and registration cannot create admin", () => {
...
test("seller approval requires admin and discount math stays integer IRR", () => {
  ...
  const seller = core.requestSeller(sellerUser.id, {
    businessName: "Miran Seller",
  });
  assert.throws(
    () => core.approveSeller(sellerUser.id, seller.id),
    /admin role/i,
  );

  const admin = makeAdmin(db, core, "admin@example.com");
  ...
  assert.deepEqual(priceProduct(product), {
    baseIrr: 1_000_000,
    finalIrr: 850_000,
    discountIrr: 150_000,
  });
  assert.equal(
    db.prepare("SELECT role FROM users WHERE id=?").get(sellerUser.id).role,
    "SELLER",
  );
  db.close();
});
...
test("orders reserve stock atomically, are idempotent, block oversell and consume inventory after verified payment", () => {
  const { db, core } = fixture();
  const admin = makeAdmin(db, core, "admin2@example.com");
  ...
  const paid = core.markOrderPaid(order.id, {
    provider: "TEST",
...
test("expired payment reservations release stock and reject late payment finalization", () => {
  const { db, core } = fixture();
  const admin = makeAdmin(db, core, "expiry-admin@example.com");
  const { user, address } = customerWithAddress(core, "expiry-buyer");
  const product = core.createProduct(admin.id, {
    title: "Reserved item",
    slug: "reserved-item",
  ...
  const order = core.createOrder(user.id, {
    addressId: address.id,
    idempotencyKey: "expiry-001",
    items: [{ productId: product.id, quantity: 1 }],
    now: startedAt,
  });

  const released = core.releaseExpiredReservations(
  ...
  assert.throws(
    () =>
      core.markOrderPaid(order.id, {
        provider: "TEST",
        now: new Date("2026-08-15T12:17:00.000Z"),
      }),
    /not payable/i,
  );
  db.close();
