          ...
          {categories.map((category) => (
            <a key={category.id} className={styles.categoryCard} href={category.href}>
              <div className={styles.categoryMedia} aria-hidden="true">
                {category.imageUrl ? <img src={category.imageUrl} alt="" loading="lazy" /> : category.imagePreset && !category.imageHidden ? <span className={styles.categorySprite} data-preset={category.imagePreset} /> : <span>{category.name.slice(0, 1)}</span>}
              </div>
              <div>
                <h3>{category.name}</h3>
